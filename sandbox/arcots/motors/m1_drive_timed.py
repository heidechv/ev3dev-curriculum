...
Authors: David Fisher and Satya Arcot.
...
#
#       speed_in_inches_per_second = 0.0108392857 * speed_in_degrees_per_second + .2857142857
#
...
#   Time (seconds) = Distance (in inches) / (0.0108392857 * speed_in_degrees_per_second + .2857142857)
#
...
def main():
    ...
    while True:
        speed = int(input("Enter a speed for the robot (0 to 700 dps): "))
        distance_i = int(input("Enter a distance to drive (inches): "))
        if speed == 0 or distance_i == 0:
            break
        left_motor.run_forever(speed_sp=speed)
        right_motor.run_forever(speed_sp=speed)
        time_s = distance_i / (0.0108392857 * speed + .2857142857)
        time.sleep(time_s)
        left_motor.stop()
